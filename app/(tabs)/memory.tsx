...
import { MOCK_MESSAGES, MOCK_ABSTRACTS } from '@/constants/MockData';
...
export default function MemoryScreen() {
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>当前没有可用终端以查询记忆</Text>
    </View>
  );

  const renderSummary = () => (
    <ScrollView contentContainerStyle={styles.scrollContent}>
      {MOCK_ABSTRACTS.map((item, index) => (
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        {/* Robot Selector Trigger */}
        <Pressable style={styles.selectorBtn} onPress={() => setPickerVisible(true)}>
          <Text style={styles.selectorText} numberOfLines={1}>{currentRobot?.robotName || '未知'}</Text>
        ...
        <View style={styles.tabSwitcher}>
          ...
          >
            <Text style={[styles.tabText, activeTab === 'DIALOGUE' && styles.tabTextActive]}>对话</Text>
          </Pressable>
        </View>
      </View>
      ...
      <Modal visible={pickerVisible} transparent animationType="fade">
        <Pressable style={styles.modalOverlay} onPress={() => setPickerVisible(false)}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>切换当前检索终端</Text>
            {robots.map(r => (
              <Pressable
                key={r.robotCode}
...
const styles = StyleSheet.create({
  ...
  abstractContentWrapper: {
    ...
    paddingLeft: 15,
    marginLeft: -21, // Overlap line with dot center roughly
  },
  abstractTime: {
    color: '#94a3b8',
    fontSize: 12,
