...
import { MOCK_FAMILY_PORTRAIT, MOCK_USER_PORTRAITS } from '@/constants/MockData';
...
export default function PortraitScreen() {
  ...
  const renderEmptyState = () => (
    <View style={styles.emptyContainer}>
      ...
      <Text style={styles.emptyText}>当前没有可用终端</Text>
    </View>
  );
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <Pressable style={styles.selectorBtn} onPress={() => setPickerVisible(true)}>
          <Text style={styles.selectorText} numberOfLines={1}>{currentRobot?.robotName || '未知终端'}</Text>
          <FontAwesome name="caret-down" size={16} color="#00e5ff" />
        </Pressable>
      </View>

      <ScrollView contentContainerStyle={styles.scrollContent}>
        {/* Family Portrait Master Card */}
        <Pressable onPress={() => router.push(`/portrait/family/${selectedRobotId}`)}>
          <LinearGradient colors={['#004e92', '#000428']} start={{x: 0, y: 0}} end={{x: 0, y: 1}} style={styles.familyCard}>
            <View style={styles.familyHeader}>
              ...
              <Text style={styles.familyTitle}>家庭综合画像</Text>
            </View>
            <Text style={styles.familyText} numberOfLines={3}>{MOCK_FAMILY_PORTRAIT.content}</Text>
            <View style={styles.familyFooter}>
              <Text style={styles.familyMeta}>Family Arch: Active</Text>
              <FontAwesome name="arrow-right" size={16} color="#00e5ff" />
            </View>
          </LinearGradient>
        </Pressable>

        <View style={styles.sectionDivider}>
          <Text style={styles.sectionTitle}>侦测到的用户图谱</Text>
        </View>

        {/* User Portrait Cards */}
        {MOCK_USER_PORTRAITS.map((u) => (
          <Pressable key={u.portraitID} onPress={() => router.push(`/portrait/user/${u.portraitID}`)}>
            <LinearGradient colors={['#003d79', '#011e41']} style={styles.userCard}>
              <View style={styles.userCardLeft}>
                <View style={styles.avatarCircle}>
                  <FontAwesome name="user-astronaut" size={28} color="#011e41" />
                </View>
              </View>
              <View style={styles.userCardBody}>
                <Text style={styles.uName}>{u.userName}</Text>
                <Text style={styles.uDetail}>{u.profession} · {u.age}岁 · {u.familyTies}</Text>
              </View>
      ...
      {/* Dropdown Modal for Robot Selection */}
      <Modal visible={pickerVisible} transparent animationType="fade">
        <Pressable style={styles.modalOverlay} onPress={() => setPickerVisible(false)}>
          <View style={styles.modalContent}>
            <Text style={styles.modalTitle}>锁定终端视觉数据源</Text>
            {robots.map(r => (
              <Pressable
                key={r.robotCode}
...
const styles = StyleSheet.create({
  ...
  emptyText: { marginTop: 20, color: '#475569', fontSize: 16 },
  header: {
