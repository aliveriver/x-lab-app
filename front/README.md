...
在机器人详情页进入“运动轨迹”，输入机器人局域网 WebSocket 地址（默认
`ws://192.168.41.2:8765/ws`）并连接。手机与机器人需在同一 Wi-Fi/可路由
局域网，Agent 的 TCP 8765 端口需可达。

页面支持设置轨迹名称、采样周期、最大时长，以及回放速度、平滑系数和
重复次数。录制停止后数据直接保存在机器人端；从列表选中轨迹后，必须
确认机器人周围安全才能开始复刻。录制和回放期间界面会禁用冲突操作，
“停止”按钮始终保留。
