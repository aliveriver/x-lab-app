...
export default function FineControlScreen() {
  ...
  const save = async () => { try { await command('arm_pose_save', { name: poseName, left_positions: left, right_positions: right, left_hand: leftHand, right_hand: rightHand }); await refresh(); } catch (e: any) { Alert.alert('保存失败', e.message); } };
